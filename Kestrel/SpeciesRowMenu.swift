...
/// one — Identify detections, life-list entries, and search suggestions — so
/// the three can never drift apart in wording, symbol, or order.
///
...
/// - **Star / Unstar** needs a bird to alert on, which a row backed by nothing
///   but a catalog entry doesn't have yet — `star` is `nil` there.
/// - **View Image** is on every row.
/// - **Delete** removes the life-list entry, so it belongs only where the row
///   *is* that entry. Passing `onDelete: nil` leaves it off.
